...
const I18N = {
  ko: {
    ...
    recordSummary: "게임 기록 요약",
    inningBreakdown: "이닝별 점수 기록",
    noInningBreakdown: "이닝별 점수 기록이 없습니다.",
    peopleSuffix: "인",
  ...
  en: {
    ...
    recordSummary: "Match Summary",
    inningBreakdown: "Inning Score Log",
    noInningBreakdown: "No inning score log.",
...
function snapshot() {
  state.history.push({
    ...
    inning: state.inning,
    active: state.active,
    inningScores: cloneInningScores(state.inningScores),
...
function restore(previous) {
  ...
  state.inningScores = cloneInningScores(previous.inningScores || []);
...
function createPlayerCard(player, index) {
  ...
  remaining.className = "remaining-score";
  remaining.textContent = Math.max(0, player.target - player.score);
  name.type = "button";
  name.className = "name-button";
  name.textContent = displayName(player.name);
  top.append(target, remaining, name);
...
function averageFor(player) {
  return formatAverage(player.score / (player.finishedAtInning || state.inning));
}

function formatAverage(value) {
  const average = Number(value);
  return Number.isFinite(average) ? average.toFixed(3) : "0.000";
}

function cloneInningScores(scores = []) {
...
function recordTurnScore(playerIndex, inning = state.inning) {
...
function inningScoresForResult() {
  const scores = cloneInningScores(state.inningScores);
...
function saveMatchResult(winner) {
  ...
  const result = {
    ...
    inningScores: inningScoresForResult(),
    players: state.players.map((player) => ({
      name: player.name,
...
function recordAverageForPlayer(player) {
  ...
  if (Number.isFinite(savedAverage)) return formatAverage(savedAverage);

  const score = Number(player.score || 0);
  const inning = Number(player.inning || player.finishedAtInning || 0);
  if (inning > 0) return formatAverage(score / inning);
  return "0.000";
}
...
function renderRecords() {
  ...
  const average =
    playerRows.length === 0
      ? "0.000"
      : formatAverage(playerRows.reduce((sum, player) => sum + Number(recordAverageForPlayer(player)), 0) / playerRows.length);
...
function createRecordRow(result, index) {
  ...
  const players = document.createElement("div");
  const deleteButton = document.createElement("button");
  const inningDetail = createInningScoreDetail(result);
  const date = new Date(result.savedAt);
  const key = resultKey(result, index);
  ...
  row.append(title, players, deleteButton, inningDetail);
  return row;
}

function createInningScoreDetail(result) {
  const detail = document.createElement("div");
  const summary = createRecordDetailSummary(result);
  const title = document.createElement("strong");
  const grid = document.createElement("div");
  const rows = normalizedInningScoreRows(result);

  detail.className = "record-inning-detail";
  title.className = "record-inning-title";
  grid.className = "record-inning-grid";
  title.textContent = t("inningBreakdown");

  if (!rows.length) {
    const empty = document.createElement("p");
    empty.className = "record-inning-empty";
    empty.textContent = t("noInningBreakdown");
    detail.append(summary, title, empty);
    return detail;
  }
  ...
  rows.forEach((row) => {
    ...
    column.className = "record-inning-column";
    inning.textContent = inningLabel(row.inning);
    column.append(inning);
  ...
  detail.append(summary, title, grid);
  return detail;
}

function createRecordDetailSummary(result) {
  ...
  summary.className = "record-detail-summary";
  title.textContent = t("recordSummary");
  meta.textContent = `${recordStandingsText(result)} · ${date.toLocaleDateString(currentLocale())} ${formatDuration(result.duration || 0)} · ${result.inning} inning · ${result.playerCount || result.players.length}${t("peopleSuffix")}`;
  result.players.forEach((player) => {
    const item = document.createElement("span");
    item.textContent = recordStatText(player);
    players.append(item);
  ...
  summary.append(title, meta, players);
  return summary;
}

function normalizedInningScoreRows(result) {
  if (!Array.isArray(result.inningScores)) return [];
  return result.inningScores
    .map((row) => ({
      inning: Number(row.inning) || 0,
...
function inningLabel(inning) {
  return state.language === "en" ? `${inning} inning` : `${inning} 이닝`;
}
...
els.recordList.addEventListener("click", (event) => {
  ...
  if (deleteButton) {
    event.stopPropagation();
    deleteResultByKey(deleteButton.dataset.deleteResult);
    return;
  }

  toggleRecordRow(event.target.closest(".record-row"));
});
els.recordList.addEventListener("keydown", (event) => {
  if (event.key !== "Enter" && event.key !== " ") return;
  const row = event.target.closest(".record-row");
  if (!row || event.target.closest("[data-delete-result]")) return;
  event.preventDefault();
  toggleRecordRow(row);
});
